...
import styles from "./DiplomesTile.module.scss";

const DiplomeTile = ({ diplome, classes, isDesktop }) => {
  const { university, image, level,title ,range, gradient, url } = diplome;
  ...
  return (
    ...
    >
      <div
        className={`h-[15rem] w-[25rem] bg-black ${styles.DiplomeTile} rounded-3xl relative p-6 flex flex-col justify-between max-w-full`}
        style={{
      ...
      >
        ...
        <Image
          ...
          alt={level}
          layout="fill"
          className={`${styles.diplomeImage} z-0`}
        />
        <div
