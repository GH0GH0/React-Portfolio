...
import { MENULINKS, DIPLOMES } from "../../constants";
import { gsap, Linear } from "gsap";
import { ScrollTrigger } from "gsap/dist/ScrollTrigger";
import DiplomeTile from "./DiplomesTile/DiplomeTile";

const Diplomes = ({ isDesktop, clientHeight }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col py- justify-center h-full">
        ...
        <div
          className={`${
            clientHeight > 600 ? "mt-12" : "mt-8"
          } flex project-wrapper w-fit seq`}
        >
          {DIPLOMES.map((diplome, index) => (
            <DiplomeTile
              classes={
                index === DIPLOMES.length - 1 ? "" : "mr-10 xs:mr-12 sm:mr-16"
              }
              diplome={diplome}
              key={diplome.title}
            />
